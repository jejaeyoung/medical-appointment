<<<<<<<< HEAD:backend/user/practitioner_controller.js
const Practitioner = require('../models/practitioner_model');

const NewUserSignUp = (req, res) => {
  Practitioner.User.create(req.body)
    .then((newUser) => {
...
const findUserById = (req, res) => {
  Practitioner.User.findOne({_id:req.params.id})
      .then((theUser) => {
          res.json({theUser})
      })
      .catch((err) => {
...
const findUserByEmail = (req, res) => {
  Practitioner.User.findOne({email:req.params.email})
      .then((theUser) => {
...
const addNewPostById = (req, res) => {
  Practitioner.User.findById({_id:req.params.id})
      .then((user) => {
        if (!user) {
...
const findPostByIdDelete = (req, res) => {
  Practitioner.User.findById(req.params.uid)
    .then((user) => {
      if (!user) {
...
const updatePostAtIndex = (req, res) => {
  Practitioner.User.findById(req.params.id)
    .then((user) => {
      if (!user) {
        return res.json({ message: 'User not found' });
      }
...
module.exports = {
    ...
    findUserById,
    updatePostAtIndex
>>>>>>>> b8d3537553919403ab4901b3bad4092bf735309f:backend/user/user_controller.js
}